...
ramp_wave(t::Real)::Real = t>=1 ? round((t%1),digits=2) : t>=0 ? t : t<=-1 ? round((1+(t%1)),digits=2) : 1+t
sawtooth_wave(t::Real)::Real =  t>=1 ? round(1-(t%1),digits=2) : t>=0 ? 1-t : t<=-1 ? round((-(t%1)),digits=2) : -t
triangular_wave(t::Real)::Real = abs(t) > 1 ? triangular_wave(abs(t)%1) : abs(t) < 0.5 ? 2abs(t) : 2-2abs(t)
square_wave(t::Real)::Real = t < 0 ? square_wave(abs(t-0.5)) : t > 1 ? square_wave(t%1) : t < 0.5 ? 1 : -1
